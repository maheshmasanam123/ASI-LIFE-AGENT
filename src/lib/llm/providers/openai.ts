...
export class OpenAIProvider implements LLMProvider {
  ...
  async validateConfig(): Promise<{ valid: boolean; error?: string }> {
    if (!this.config.apiKey) {
      return { valid: false, error: 'OpenAI API key not configured. Set OPENAI_API_KEY environment variable or provide in config.' };
    }
    ...
    try {
      ...
      return { valid: false, error: `OpenAI validation failed: ${error instanceof Error ? error.message : String(error)}` };
    }
  }
