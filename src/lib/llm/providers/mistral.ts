...
export class MistralProvider implements LLMProvider {
  ...
  async validateConfig(): Promise<{ valid: boolean; error?: string }> {
    if (!this.config.apiKey) {
      return { valid: false, error: 'Mistral API key not configured. Set MISTRAL_API_KEY environment variable or provide in config.' };
    }
    try {
      await this.client.models.list();
      return { valid: true };
    } catch (error) {
      return { valid: false, error: `Mistral validation failed: ${error instanceof Error ? error.message : String(error)}` };
    }
  }
  ...
  private mapMessage(msg: LLMMessage): OpenAI.Chat.Completions.ChatCompletionMessageParam {
    ...
    if (msg.tool_calls && msg.tool_calls.length > 0) { return { role: 'assistant', content: msg.content || null, tool_calls: msg.tool_calls.map(tc => ({ id: tc.id, type: 'function' as const, function: { name: tc.function.name, arguments: tc.function.arguments } })) }; }
...
export function createMistralProvider(config?: LLMProviderConfig): MistralProvider { return new MistralProvider(config); }