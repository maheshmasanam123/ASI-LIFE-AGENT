...
export class OllamaProvider implements LLMProvider {
  ...
  private async fetchModels(): Promise<void> {
    ...
    try {
      ...
      this.models = modelsList.data.map(m => m.id).filter(id => !id.includes('embed'));
      if (this.models.length === 0) this.models = ['llama3.1', 'mistral', 'codellama', 'gemma2'];
      this.modelsFetched = true;
    } catch {
      this.models = ['llama3.1', 'mistral', 'codellama', 'gemma2'];
      this.modelsFetched = true;
    }
  }
  ...
  async validateConfig(): Promise<{ valid: boolean; error?: string }> {
    try {
      ...
      return { valid: false, error: `Ollama validation failed: ${error instanceof Error ? error.message : String(error)}. Is Ollama running at ${this.config.baseURL}?` };
    }
  }
...
export function createOllamaProvider(config?: LLMProviderConfig): OllamaProvider { return new OllamaProvider(config); }